...
- **集群 / 分布式就绪**（v0.1.5）：开关 `code-insight.cluster.enabled`（`CLUSTER_ENABLED`）；Leader 选举（`ci:leader:*`）、`SELECT … FOR UPDATE SKIP LOCKED` 任务认领、Redis Set `ci:permits:*` 并发控制、Pub/Sub 配置广播、共享存储卷。详见 [docs/cluster-readiness.md](./docs/cluster-readiness.md)。
...
7. 集群 / 分布式：在多节点环境验证 `code-insight.cluster.enabled=true` 全链路（Leader 选举、Redis 许可、共享卷、Pub/Sub 广播）。
